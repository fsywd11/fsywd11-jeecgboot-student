<template>
  <div class="account-padding" :class="[`${prefixCls}`]">
    ...
    <div class="account-data border-bottom">
      <!-- 学生信息 -->
      <div class="account-detail" style="width: 100%">
        <div class="font-size-15 font-bold font-color-gray" style="margin-bottom: 16px">学生信息</div>
        <div class="grid-layout">
          <div class="margin-bottom-10 font-size-13">
            <span class="gray-75 item-label">学号</span>
            <span class="gray-3">{{ studentInfo?.studentNo || '未填写' }}</span>
          </div>
          <!-- 学院：字典转换 -->
          <div class="margin-bottom-10 font-size-13">
            <span class="gray-75 item-label">学院</span>
            <span class="gray-3">{{ studentInfo?.college || '未填写' }}</span>
          </div>
          <!-- 专业：字典转换 -->
          <div class="margin-bottom-10 font-size-13">
            <span class="gray-75 item-label">专业</span>
            <span class="gray-3">{{ studentInfo?.major || '未填写' }}</span>
          </div>
          <!-- 年级：字典转换 -->
          <div class="margin-bottom-10 font-size-13">
            <span class="gray-75 item-label">年级</span>
            <span class="gray-3">{{ studentInfo?.grade || '未填写' }}</span>
          </div>
          <div class="margin-bottom-10 font-size-13">
            <span class="gray-75 item-label">班级</span>
            <span class="gray-3">{{ studentInfo?.className || '未填写' }}</span>
          </div>
          <div class="margin-bottom-10 font-size-13">
            <span class="gray-75 item-label">身份证号</span>
            <span class="gray-3">{{ studentInfo?.studentIdCard || '未填写' }}</span>
          </div>
        </div>
      </div>
    </div>
...
<script lang="ts" setup>
  ...
  import { userEdit, queryByUserId, getUserData, queryNameByCodes } from './UserSetting.api';
  ...
  function getcollege(val) {
    let findOption = COLLEGEOption.find((item) => String(item.value) === val);
    let college = '未填写';
    if (findOption) {
      college = findOption.text;
    }
    return college;
  }
  ...
  function getMajor(val) {
    let findOption = MAJOROption.find((item) => String(item.value) === val);
    let major = '未填写';
    if (findOption) {
      major = findOption.text;
    }
    return major;
  }
  ...
  function getGrade(val) {
    let findOption = GRADEOption.find((item) => String(item.value) === val);
    let grade = '未填写';
    if (findOption) {
      grade = findOption.text;
    }
    return grade;
  }
  ...
  async function getStudentInfo(userId) {
    ...
    try {
      // 2. 传递userId参数到接口（关键修正：补充params参数）
      const res = await queryByUserId({ userId });

      // 3. 响应处理（适配后端Result格式）
      if (res && res.success) {
        // 匹配后端Result.OK的success字段
        res.result.college = getcollege(res.result.college);
        res.result.major = getMajor(res.result.major);
        res.result.grade = getGrade(res.result.grade);
        studentInfo.value = res.result; // 后端返回的EduStudent数据
      } else {
        // 处理接口返回失败的情况（如未找到对应数据）
        studentInfo.value = null;
        createMessage.error('未找到对应学生信息');
      }
    } catch (error) {
      // 4. 网络错误/接口异常处理
      console.error('获取学生信息异常：', error);
      studentInfo.value = null;
      createMessage.error('网络异常，获取学生信息失败');
    }
  }
</script>

  ...
  <div class="account-padding" :class="[`${prefixCls}`]">
    ...
    <div class="account-data border-bottom">
      ...
      <div class="account-detail" style="width: 100%">
        <div class="font-size-15 font-bold font-color-gray" style="margin-bottom: 16px">教师信息</div>
...
<script lang="ts" setup>
  ...
  //department教研室
  const departmentOption = getDictItemsByCode('DEPARTMENT') || [
    { text: '牛马部门', value: '1' },
    { text: '超人部门', value: '2' },
    { text: '管理部门', value: '3' },
  ]
  //title职称
  const titleOption = getDictItemsByCode('TITLE') || [
    { text: '教授', value: '1' },
    { text: '副教授', value: '2' },
    { text: '讲师', value: '3' },
  ]
  ...
  function getDepartment(val) {
    let findOption = departmentOption.find((item) => String(item.value) === val);
    let department = '未填写';
    if (findOption) {
      department = findOption.text;
    }
    return department;
  }
  //获取职称
  function getTitle(val) {
    let findOption = titleOption.find((item) => String(item.value) === val);
    let title = '未填写';
    if (findOption) {
      title = findOption.text;
    }
    return title;
  }
    ...
    getUserData().then(async (res) => {
      if (res.success) {
        if (res.result) {
          ...
          await getTeacherInfo(userInfo.value.id);
  ...
  async function getTeacherInfo(userId) {
    ...
    try {
      ...
      if (res && res.success) {
        ...
        res.result.department = getDepartment(res.result.department);
        res.result.title = getTitle(res.result.title);
        TeacherInfo.value = res.result; // 后端返回的EduStudent数据
      } else {
        // 处理接口返回失败的情况（如未找到对应数据）
        TeacherInfo.value = null;
        createMessage.error('未找到对应教师信息');
      }
    } catch (error) {
      // 4. 网络错误/接口异常处理
      console.error('获取教师信息异常：', error);
      TeacherInfo.value = null;
      createMessage.error('网络异常，获取教师信息失败');
    }
  }
</script>
